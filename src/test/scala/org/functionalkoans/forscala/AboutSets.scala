...
class AboutSets extends KoanSuite {
  ...
  koan("Sets may be of mixed type") {
    val mySet: Set[Any] = Set("Michigan", "Ohio", 12)
  ...
  koan("Set elements can be removed easily") {
    ...
    aNewSet("Michigan") should be(false)
  }
  ...
  koan("Sets can be iterated easily") {
    ...
    sum should be(17)
    sum should be(mySet.reduce(_ + _))
  }
  ...
  koan("Two sets can be joined as their union easily") {
    ...
    aNewSet2.equals(Set("Michigan", "Wisconsin", "Ohio", "Iowa", "Minnesota")) should be(true)
  }
  ...
  koan("A set is either a subset of another set or it isn't") {
    ...
    mySet2 subsetOf mySet1 should be(false) // Minnesota is not present on set 1
