...
class AboutMaps extends KoanSuite {
  ...
  koan("Map values can be iterated") {
    ...
    val mapValues: Iterable[String] = myMap.values
...
//    mapValues.contains("Illinois") should be (true)
  ...
  koan("Maps insertion with duplicate key updates previous entry with subsequent value") {
    val myMap: Map[String, String] = Map("MI" -> "Michigan", "OH" -> "Ohio", "WI" -> "Wisconsin", "MI" -> "Meechigan")
  ...
  koan("Map keys may be of mixed type") {
    val myMap: Map[Any, String] = Map("Ann Arbor" -> "MI", 49931 -> "MI")
...
//    intercept[NoSuchElementException] {
      myMap.contains("MI") should be(false)
//    }
  }
  ...
  koan("Map equivalency is independent of order") {
    ...
    myMap1 eq myMap2 should be(false) // reference comparison
    myMap1 == myMap2 should be(true)

    myMap1.keys.head should be("MI")
    myMap2.keys.head should be("WI")

    val myMap3 = TreeMap("MI" -> "Michigan", "OH" -> "Ohio", "WI" -> "Wisconsin", "IA" -> "Iowa")
    val myMap4 = TreeMap("WI" -> "Wisconsin", "MI" -> "Michigan", "IA" -> "Iowa", "OH" -> "Ohio")

    myMap3 == myMap4 should be(true)

    myMap3.keys.head should be("IA")
    myMap4.keys.head should be("IA")
  }
}
