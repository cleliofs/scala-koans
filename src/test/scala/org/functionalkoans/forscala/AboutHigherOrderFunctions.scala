...
class AboutHigherOrderFunctions extends KoanSuite {
  ...
  koan("function returning another function") {
    def addWithoutSyntaxSugar(x: Int) = {
      def closure = new Function1[Int, Int]() {
  ...
  koan(
    ...
    def makeUpper2(xs: List[String]) = xs map { x => x.toUpperCase}
    def makeUpper3(xs: List[String]) = xs map(x => x.toUpperCase)
    def makeUpper4(xs: List[String]) = xs.map( (x: String) => x.toUpperCase)
  ...
  koan("Currying is a technique to transform function with multiple parameters to function with one parameter") {
    ...
    def multiplyCurried = (multiply _).curried
    val intToIntToInt: () => (Int) => (Int) => Int = multiplyCurried _
    val intToIntToInt2: Function0[Function1[_, Function1[_, _]]] = multiplyCurried _
    (multiplyCurried _).isInstanceOf[() => Function1[_, Function1[_ , _]]] should be(true)
    (multiplyCurried _).isInstanceOf[Function0[Function1[_, Function1[_, _]]]] should be(true)
    multiplyCurried(3)(2) should be(6)
  }

  koan("Currying allows you to create specialized version of generalized function") {
    def customFilter(p: Int => Boolean)(xs: List[Int]) = {
      xs filter p
    }
